...
def mock_lima_graph(
    ...
    Returns:
        data: torch_geometric.data.Data con:
            - x: Features estáticos de nodos [N, 10]
            - x_temporal: Features temporales [N, T, 10]
            - edge_index: Conectividad del grafo [2, E]
            - edge_attr: Features de aristas [E, 2] (distancia, tipo_conexión)
            - y: Target — tiempos de viaje por arista [E]
            - train_mask: Máscara de aristas para entrenamiento [E]
            - test_mask: Máscara de aristas para test [E]
            - node_types: Modo de transporte por nodo [N]
            - node_line: Índice de línea por nodo [N]
    ...
    # =================================================================
    indices = rng.permutation(num_edges)
    ...
    print("Status Mock: Train/test split completado")

    # =================================================================
    # Paso 7: Construir Data de PyG
    # =================================================================
    """
    data.x features (10 dims):
    ...
    edge_attr = torch.tensor(
        list(zip(edge_distances, edge_types)), dtype=torch.float32
    )
    edge_attr = np.array(list(zip(edge_distances, edge_types)), dtype=np.float32)

    data = Data(
        x=torch.tensor(x_static, dtype=torch.float32),
        edge_index=edge_index,
        edge_attr=edge_attr,
        y=torch.tensor(travel_times, dtype=torch.float32),
...
if __name__ == "__main__":
    ...
    # print(f"  Train: {metadata['train_edges']}, Test: {metadata['test_edges']}")
    ...
    # print(f"Tiempos de viaje: {metadata['travel_time_stats']}")
