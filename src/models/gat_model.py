"""GAT para predicción de tiempos de viaje sobre aristas.

Utiliza un stack de GATConv (Graph Attention Network) como encoder de nodos,
luego concatena embeddings de pares (src, dst) junto con features de arista
...
class TravelTimeGAT(nn.Module):
    ...
    Arquitectura:
        1. Stack de GATConv: in_channels → hidden_dim (num_layers capas)
           con multi-head attention (los heads se concatenan en capas
           intermedias y se promedian en la última capa).
    ...
    def encode(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        """Encoder: stack de GATConv con BatchNorm, ELU y dropout."""
